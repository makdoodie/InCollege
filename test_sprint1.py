...
def test_register_fail_password(system_instance, capfd, temp_remove_accounts):
  ...
  account = system_instance.cursor.fetchone()
  assert result is False and std.out.strip(
  ) == password_warnings['length'] and account is None
  ...
  account = system_instance.cursor.fetchone()
  assert result is False and std.out.strip(
  ) == password_warnings['uppercase'] and account is None
  ...
  account = system_instance.cursor.fetchone()
  assert result is False and std.out.strip(
  ) == password_warnings['digit'] and account is None
  ...
  account = system_instance.cursor.fetchone()
  assert result is False and std.out.strip(
  ) == password_warnings['special'] and account is None
